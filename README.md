...
- **镜像模式**（v50/v51）：强制全屏 / GPU 渲染的程序（相机客户端、播放器等）用 DWM 缩略图把画面**实时**缩放进槽位，**鼠标点击会转发给原程序**；自动按「悬浮 → 嵌入 → 镜像」逐级升级
- **不用手填进程名**（v51）：把任意程序拖到槽位、或用 F8 捕获，会自动把它的进程加进「目标进程」
...
当前 **v51**：多应用 + 全屏/无边框/置顶/最小化状态处理 + 消除闪烁 + **镜像模式（DWM 缩略图，实时、可点击）**，并保留 v46–v50 的全部修复。
