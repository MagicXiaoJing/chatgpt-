...
把**多个 ChatGPT 桌面窗口**收进同一个窗口、按格子排好的小工具。
给同时开着好几个 ChatGPT 干活的人用：不用再来回切窗口找"那个在等我回话的"。
...
1. 打开 **ChatGPT 桌面版**（要几个窗口就开几个）
2. 双击 **`ChatGPTWorkspace.exe`**
3. 设好「行 / 列」，程序会**自动**把发现的 ChatGPT 窗口填进空格子；满意后点「保存布局」
...
- **自动认领**：启动时把 ChatGPT 窗口按顺序填进空槽位，窗口标题变化也不会丢绑定
...
当前 **v46**：修复了悬浮模式显示、输入丢焦点、面板闪黑、误触拖拽、**弹窗被 ChatGPT 窗口遮挡点不到按钮**等问题，并加入启动自动认领与最小化窗口自动还原。
